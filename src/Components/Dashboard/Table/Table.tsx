...
const Table=()=>{
    ...
    const ClicksetManualPoint_X=()=>{if(manualPoint_X==='0'){setManualPoint_X("")} else return}
    const ClicksetManualPoint_Y=()=>{if(manualPoint_Y==='0'){setManualPoint_Y("")} else return}
    const ClicksetManualPoint_SETA=()=>{if(manualPoint_SETA==='0'){setManualPoint_SETA("")} else return}

    const blursetManualPoint_X=()=>{if(manualPoint_X===''){setManualPoint_X("0")} else return}
    const blursetManualPoint_Y=()=>{if(manualPoint_Y===''){setManualPoint_Y("0")} else return}
    const blursetManualPoint_SETA=()=>{if(manualPoint_SETA===''){setManualPoint_SETA("0")} else return}
    ...
    const getTablepointValues = async ()=>{
            const res = await fetch (`http://127.0.0.1:8001/tablemode/getpoints/${selectedTable}`)
            const data=JSON.parse(await res.json())
            setSaved_X(data[1])
            setSaved_Y(data[2])
    ...
    const setNewPoints=async (sentPointsType:string)=>{
      await fetch(`http://127.0.0.1:8001/tablemode/updatepoints/${selectedTable}`, {
        method: "PATCH",
        headers: {
    ...
    return (
        ...
        <div className="grid grid-cols-1 grid-rows-[200px_1fr] border-2">
          ...
          <div className="border-amber-400 border-2 flex justify-center items-start">
            ...
            <div className="relative border-2 w-1/2 h-full">
              ...
              <div className="flex items-center gap-8 px-10 m-9">
                  ...
                  value={manualPoint_X} onBlur={blursetManualPoint_X} onFocus={ClicksetManualPoint_X} onChange={e => setManualPoint_X(e.target.value)} />
              </div>
              <div className="flex items-center gap-8 px-10 m-9 ">
                <span className="font-bold text-xl mr-10">Y:</span>
                <input className="w-20 h-7 border rounded-md bg-white text-center"
                  value={manualPoint_Y} onBlur={blursetManualPoint_Y} onFocus={ClicksetManualPoint_Y} onChange={e => setManualPoint_Y(e.target.value)} />
              </div>
              <div className="flex items-center gap-3 px-10 m-4 ">
                <span className="font-bold text-xl mr-10">Theta:</span>
                <input className="w-20 h-7 border rounded-md bg-white text-center"
                  value={manualPoint_SETA} onBlur={blursetManualPoint_SETA} onFocus={ClicksetManualPoint_SETA} onChange={e => setManualPoint_SETA(e.target.value)} />
              </div>
  
              <div className="flex justify-center">
                <button className="border-2 p-2 mt-6 rounded-lg" onClick={() => setNewPoints("Main")}>
                  Set Main Table
                </button>
              </div>
              <div className="flex justify-center items-center ">
                <button className="border-2 mt-6 m-4 p-2 rounded-lg" onClick={() => setNewPoints("BackUp_1")}>
                  Set backup 1
                </button>
                <button className="border-2 m-4 mt-6 p-2 rounded-lg" onClick={() => setNewPoints("BackUp_2")}>
                  Set backup 2
                </button>
                <button className="border-2 m-4 mt-6 p-2 rounded-lg" onClick={() => setNewPoints("BackUp_3")}>
                  Set backup 3
                </button>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 border-2">
            <div>
              <h1 className="font-bold h-10 pl-35 pt-2 px-2 bg-amber-600 mb-1 ">
                Main Point
              </h1>
    
              <div className="space-y-3 pb-1">
                <div className="flex items-center gap-3  pl-10">
                  <span className="font-bold text-xl mr-10.5 bg-amber-300">X:</span>
                  <div className="w-20 h-7 border rounded-md bg-white flex items-center justify-center">
                    {saved_X}
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3 pl-10">
                  <span className="font-bold text-xl mr-10.5 text-green-400">
                    Y:
                  </span>
                  <div className="w-20 h-7 border rounded-md bg-white flex items-center justify-center">
                    {saved_Y}
            ...
            <div>
              ...
              <div className="flex items-center gap-30 pl-10">
                ...
                <div className="flex items-center gap-30 pl-10">
                  <span className="font-bold text-xl mr-10.5">
                    Y:
                  </span>
                  <div className="w-20 h-7 border rounded-md bg-white flex items-center justify-center">
                    {B1_saved_Y}
